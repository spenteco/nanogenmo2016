...
def texts_to_pdf_latex(text_folder, file_name, author_name_subs_folder, temp_folder, pdf_folder, template_folder, latex_folder):
    ...
    title_page_template = Template(filename=template_folder + 'title_page.tex')
            ...
            if p[0] in ['PRESERVE FORMAT', 'VERSE']:
                ...
                for line_n, line in enumerate(lines):
                    ...
                    if line_n == len(lines) - 1:
                        tex_body.append(indent_string + fix_small_caps(line, True))
                    else:
                        tex_body.append(indent_string + fix_small_caps(line, True) + r' \\')
                ...
                if p[0] in ['INDENTED PROSE', 'PROSE', 'SINGLE']:
                    ...
                    tex_body.append(r'\par')
                    tex_body.append(fix_small_caps(p_text, True))
                    ...
                    if p[0] in ['HEADING']:
                        ...
                        tex_body.append(r'\textbf{' + fix_small_caps(fixed_p, True) + '}')
    ...
    f = codecs.open(latex_folder + file_name.replace('.txt', '.pdf.tex'), 'w', encoding='utf-8')
